...
export async function generateMetadata(
  ...
  try {
    ...
    const { data } = await axios.get(`${apiBase}/home-page?fields[0]=title&populate[metaData][populate][metaImage][fields][0]=url`)

    const metadata = {
      title: data.data.attributes.metaData.metaTitle,
      description: data.data.attributes.metaData.metaDescription,
      openGraph: {
        images: [`${process.env.NEXT_PUBLIC_IMAGE_BASE}${data.data.attributes.metaData.metaImage.data.attributes.url}`]
      },
    }
