...
export default async function Album({ params }: Props) {
  const { data } = await axios.get(
    `${process.env.NEXT_PUBLIC_API_BASE}/home-page?fields[0]=title&populate[gallery][on][element.album][filters][title]=${params.title}&populate[gallery][on][element.album][populate][images][fields][0]=url&populate[gallery][on][element.album][populate][images][fields][1]=formats`
  );
  ...
  return (
    <section className="w-full h-full flex flex-col gap-12">
      <div className="flex flex-col gap-5">
        <h2 className="text-2xl capitalize">{album.title}</h2>
